...
# Usage: ./vc_MuTect2_pipeline.sh <normal sample name> <tumor sample name>
...
gatk Mutect2 \
    ...
    --output 3_var/${TUMOR}_mn_${NORMAL}.raw.vcf.gz
#    --bam-output 3_var/${TUMOR}.HC.bam \
#    --panel-of-normals pon.vcf.gz \

if [ ! -f 3_var/filter_files/${NORMAL}.pileups.table ]
then
gatk GetPileupSummaries \
    --input 2_pre/${NORMAL}.bam \
    --variant ${GRMBA} \
    --intervals ${GRMBA} \
    --output 3_var/filter_files/${NORMAL}.pileups.table
fi
...
gatk CalculateContamination \
    ...
    --matched-normal 3_var/filter_files/${NORMAL}.pileups.table \
    --output 3_var/filter_files/${TUMOR}.contamination
fi

gatk FilterMutectCalls \
    --variant 3_var/${TUMOR}_mn_${NORMAL}.raw.vcf.gz \
    --contamination-table 3_var/filter_files/${TUMOR}.contamination \
    --output 3_var/${TUMOR}_mn_${NORMAL}.filtered.vcf.gz

rm 3_var/${TUMOR}_mn_${NORMAL}.raw.vcf.gz 3_var/${TUMOR}_mn_${NORMAL}.raw.vcf.gz.tbi Mutect2FilteringStats.tsv
...
gatk FilterByOrientationBias \
    --variant 3_var/${TUMOR}_mn_${NORMAL}.filtered.vcf.gz \
    --pre-adapter-detail-file 3_var/filter_files/${TUMOR}.pre_adapter_detail_metrics \
    --output 3_var/${TUMOR}_mn_${NORMAL}.vcf.gz

rm 3_var/${TUMOR}_mn_${NORMAL}.filtered.vcf.gz 3_var/${TUMOR}_mn_${NORMAL}.filtered.vcf.gz.tbi 3_var/${TUMOR}_mn_${NORMAL}.vcf.gz.summary
